...
| 교육기간(23.06.26 ~ 23.12.06) 중 프로젝트이다.

[멀티잇] 데이터분석&엔지니 취업캠프(28회차), 
문제해결 빅데이터 활용 프로젝트 최우수상 수상(1위) 
...
## 머신러닝
[머신러닝/XGBoost(최종).ipynb](https://github.com/mizima1015/multi_semi_project/blob/main/%EB%A8%B8%EC%8B%A0%EB%9F%AC%EB%8B%9D/XGBoost(%EC%B5%9C%EC%A2%85).ipynb)
...
## 웹 개발
![README_image](README_image/기타/웹.png)
- 대시보드틑 하이차트를 히용해 그래프를 그렸고 각 그래프에 맞게 추가적으로 전처리를 조금씩 했다.
    - 하이차트란 자바스크립트를 통해 동적그래프를 대시보드에 나타내기 위해 사용하는 라이브러리이다.
- 웹 서비스 부분은 학습데이터 pkl파일을 불러와 출발/도착역, 시간대와 같은 변수를 넣으면 지나가는 노선들의 혼잡도의 평균을 계산해 보여준다.
- 2호선 순환열차는 시계방향, 반시계 방향에 따라 가까운 방향으로 혼잡도를 계산하도록 했고, 2호선의 꼬다리처럼 튀어나온 성수지선과 신정지선, 5호선의 마천지선은 갈아타야하는 호선이기에 별개의 호선으로 구분했다.
- 다른 호선들도 마찬가지로 특이점은 그에 맞춰 별도의 로직을 구현했다.

## 데이터 정의서와 부록
[최총산출물/최종발표ppt.pdf](https://github.com/mizima1015/multi_semi_project/blob/main/%EC%B5%9C%EC%B4%9D%EC%82%B0%EC%B6%9C%EB%AC%BC/%EC%B5%9C%EC%A2%85%EB%B0%9C%ED%91%9Cppt.pdf)
- 최종 ppt 후반부의 부록에서 확인 가능하다.
## 팀원
